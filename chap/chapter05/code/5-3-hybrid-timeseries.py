...
# -*- coding: utf-8 -*-
"""
제5장: LSTM-Transformer 하이브리드 시계열 모델 구현
...
class LSTMTransformerHybrid:
    ...
    def build_model(self, input_features):
        ...
        self.model.compile(
            optimizer=Adam(learning_rate=0.0005),  # 더 안정적인 학습률
            loss='mse',
            metrics=['mae']
        )
...
def generate_policy_timeseries_data(n_samples=1000):
    ...
    Returns:
        DataFrame: 시계열 데이터
    """
    print("📊 정책 시계열 시뮬레이션 데이터 생성 (고성능 모델용)")
    ...
    print(f"   - 강한 선형관계와 예측가능성 확보")

    # 상관관계 확인
    correlation = data.corr()['policy_effect']
    print(f"   - 정책강도와 상관관계: {correlation['policy_intensity']:.3f}")
    print(f"   - 경제지표와 상관관계: {correlation['economic_indicator']:.3f}")
...
def main():
    ...
    메인 실행 함수
    """
    print("🚀 LSTM-Transformer 하이브리드 시계열 모델 구현")
    print("=" * 60)
    ...
    # 3. 모델 초기화 및 학습 (개선된 파라미터)
    model = LSTMTransformerHybrid(
        sequence_length=60,  # 더 긴 시퀀스로 패턴 학습 향상
        lstm_units=128,      # 더 많은 유닛으로 복잡성 증가
        num_heads=8,         # 더 많은 attention head
        transformer_dim=128, # 더 큰 transformer 차원
        dropout_rate=0.2     # 적절한 정규화
    )
